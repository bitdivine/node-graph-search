...
function depth_first(problem){
	var visited = new Set();                           // Names of tested nodes.
	var final_set = new Set(problem.terminal_states);  // Names of destination nodes.
	var boundary = problem.initial_states.map((name) => {return {name:name,parent:null,cost:0};}); // queue.  Add with push.  Remove with pop.
...
if (!module.parent){
	console.log(depth_first(require(process.argv[2]  || './data')));
}
